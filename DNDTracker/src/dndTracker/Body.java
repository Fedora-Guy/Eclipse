...
public class Body extends Component implements MouseListener, KeyListener{
		...
		// When you clear a slot, make it so the images disappear. (possible problem)

	int size = 612;
    Color background = new Color(200, 200, 200);
    ArrayList <ButtonObj> 		list = new ArrayList <>();
    ArrayList <MonsterSheet> monster = new ArrayList <>();
    ...
    public void createEnemy () {
    	...
    	while(groupNumber == 0) {
    		try {
    			String response = JOptionPane.showInputDialog("How many enemies in the group?");
    			if(response == null) {
    				return;
    			} else {
    				groupNumber = Integer.parseInt(response);
    			}
        		
        	} catch (NumberFormatException e) {
        		JOptionPane.showMessageDialog(null, "Input numbers, not String", "Problem detected", JOptionPane.INFORMATION_MESSAGE);
        	}
    	}
    	...
    	if(monster.size() + groupNumber <= 14) {
    		...
    		JOptionPane.showMessageDialog(getParent(), "You have too many enemies \nRemove some nerd", "Problem detected", JOptionPane.INFORMATION_MESSAGE);
    	}
    ...
    public void disEnemies(Graphics2D g2) {
    	...
    	for(int temp = 0; temp < monster.size(); temp++) {
    		...
    		BufferedImage img = null;
            try {
            	URL url = EnemyMain.class.getResource("/Images/" + monster.get(temp).name + ".jpg");
            	img = ImageIO.read(url);
            } catch (IOException e) {
            	e.printStackTrace();
            ...
            if(img != null) {
            	g2.drawImage(img, (160*(temp%7))+15, (210*(temp/7))+15, 135, 80, null);
            	g2.setFont(new Font("Serif", Font.PLAIN, 20));
            	g2.drawString(monster.get(temp).name + " - " + (monster.get(temp).number), 					(160*(temp%7))+20, (210*(temp/7))+110);
            	g2.setFont(new Font("Serif", Font.PLAIN, 30));
            } else {
    		...
    		g2.drawString("HP: " + monster.get(temp).currentHealth + "/" + monster.get(temp).maxHealth, (160*(temp%7))+20, (210*(temp/7))+135);
    		g2.setColor(Color.BLACK);
    		g2.drawString("AC: " + monster.get(temp).armorClass, 										(160*(temp%7))+20, (210*(temp/7))+160);
    	}
    }
    ...
    public void keyTyped(KeyEvent e) {
       System.out.println(e.getKeyChar());
        if(e.getKeyChar() == 'c') {
        	System.out.println("Clear All typed");
